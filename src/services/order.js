...
const { sortObject, STATUS_ORDER, ORDERS_KEY, checkStatusOrder } = require('../utils/order');
...
const mGetProductOrder = async (res, dataOrder, messageResponseID, timestamp, status) => {
    let messageResponse = new MessageResponses();
    messageResponse.setId(messageResponseID);
    messageResponse.setCreatedAt(timestamp);
...
class OrderService {
    ...
    getOrdersByStatus = async (req, res) => {
        ...
        if (status === undefined) {
            ...
            return res.send({ message: messageResponse.toJSON(), statusCode: 400, code: "order/missing-status", timestamp });
        }
        let isNumberStatus = isNumber(status);
        if (!isNumberStatus) {
            messageResponse.setStatusCode(400);
            messageResponse.setCode("order/status-nan");
            messageResponse.setContent("status not a number.");
            return res.send({
                message: messageResponse.toJSON(),
                statusCode: 400,
                ordersDetail: {},
                code: "order/status-nan",
                timestamp
            });
        }

        let statusValue = parseInt(status)
        let isValidStatus = checkStatusOrder(statusValue);
        if (!isValidStatus) {
            messageResponse.setStatusCode(400);
            messageResponse.setCode("order/status-invalid-value");
            messageResponse.setContent("status invalid value.");
            return res.send({
                message: messageResponse.toJSON(),
                statusCode: 400,
                code: "order/status-invalid-value",
                timestamp
            });
        }
        ...
        try {
            ...
            let dataOrderWaiting = await OrderModel.orderModel.find(filters).lean();
            let mData = await mGetProductOrder(res, dataOrderWaiting, id, timestamp, statusValue);
            dataProductOrders[ORDERS_KEY[statusValue]] = mData;
            messageResponse.setStatusCode(200);
            messageResponse.setCode("order/get-data-order-success");
            messageResponse.setContent("Get data order success.");
            return res.send({
                message: messageResponse.toJSON(),
                statusCode: 200,
                ordersDetail: dataProductOrders,
                code: "order/get-data-order-success",
                timestamp
            });
        } catch (e) {
            ...
            console.log(e.message.toString());
            messageResponse.setStatusCode(400);
            messageResponse.setCode("order/get-data-order-failed");
            messageResponse.setContent(e.message.toString());
            return res.send({
                message: messageResponse.toJSON(),
                statusCode: 400,
                code: "order/get-data-order-failed",
                timestamp
            });
        }
    }
